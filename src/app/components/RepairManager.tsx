...
export default function RepairManager() {
  const [formData, setFormData] = useState<RepairFormData>({ name: '', enName: '', description: '', enDescription: '', images: [], published: false, metadata: '' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId ? `/api/repairs/${editingId}` : '/api/repairs'; const method = editingId ? 'PUT' : 'POST'
      const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(repairData) })
      if (!response.ok) { throw new Error(`Failed to ${editingId ? 'update' : 'create'} repair`) }
      const resultRepair = await response.json()
      if (editingId) { setRepairs(repairs.map((item) => (item.id === editingId ? resultRepair : item))); setSuccessMessage('Oprava uspesne upravena!') } else { setRepairs([...repairs, resultRepair]); setSuccessMessage('Oprava uspesne vytvorena!') }
      resetForm()
    } catch (err) { setError(err instanceof Error ? err.message : 'An unknown error occurred') } finally { setLoading(false) }
  }

  const fetchRepairs = async () => { try { setLoading(true); setError(null); const response = await fetch('/api/repairs'); if (!response.ok) { throw new Error('Failed to fetch repairs') }; const data = await response.json(); setRepairs(data) } catch (err) { setError(err instanceof Error ? err.message : 'An unknown error occurred') } finally { setLoading(false) } }
  ...
  const handleDrop = async (dropIndex: number) => {
    ...
    const updatedRepairs = [...repairs]; const [draggedItem] = updatedRepairs.splice(draggedIndex, 1); updatedRepairs.splice(dropIndex, 0, draggedItem)
    const repairsWithNewOrder = updatedRepairs.map((item, index) => ({ ...item, order: index + 1 }))
    setRepairs(repairsWithNewOrder); setDraggedIndex(null)
    try { const response = await fetch('/api/repairs/reorder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ repairs: repairsWithNewOrder.map((r) => ({ id: r.id, order: r.order })) }) }); if (!response.ok) { throw new Error('Failed to update order') }; setSuccessMessage('Poradie uspesne upravene!'); setTimeout(() => setSuccessMessage(null), 3000) } catch { setError('Nepodarilo sa upravit poradie. Prosim obnovte stranku.'); fetchRepairs() }
  }
  ...
  return (
    <div className="p-8" style={{ background: '#1a1510', border: '1px solid #2a2018' }}>
      <h2 className="text-2xl italic mb-6" style={{ fontFamily: 'var(--font-cormorant)', color: '#faf8f5' }}>
        {editingId ? 'Upravit opravu' : 'Vytvorit opravu'}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading} className="flex-1 py-3 px-6 tracking-[0.15em] uppercase font-bold transition-colors disabled:opacity-50 cursor-pointer" style={{ fontFamily: 'var(--font-poiret-one)', background: '#e80e19', color: '#faf8f5' }}>
            {loading ? 'Ukladam...' : editingId ? 'Upravit opravu' : 'Vytvorit opravu'}
          </button>
          {editingId && (
      ...
      <div className="mt-8">
        <button onClick={fetchRepairs} disabled={loading} className="w-full py-3 px-6 tracking-[0.15em] uppercase font-bold transition-colors disabled:opacity-50 cursor-pointer" style={{ fontFamily: 'var(--font-poiret-one)', border: '1px solid #2a2018', color: '#9b8f84', background: 'transparent' }}>
          {loading ? 'Nacitavam...' : 'Vsetky opravy'}
        </button>
        ...
        {repairs.length > 0 && (
          <div className="mt-6 space-y-3">
            <div className="flex items-center gap-4 pb-3" style={{ borderBottom: '1px solid #2a2018' }}>
              ...
              <span style={{ fontFamily: 'var(--font-poiret-one)', color: '#8b6914', letterSpacing: '0.15em', fontSize: '13px', textTransform: 'uppercase' }}>Vsetky opravy ({repairs.length})</span>
            </div>
            <p className="text-sm mb-2" style={{ color: '#9b8f84' }}>Presunте opravy tahanim pre zmenu poradia zobrazenia</p>

            {[...repairs]
              .sort((a, b) => { if (a.order !== b.order) return a.order - b.order; return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime() })
              .map((repair, index) => (
                <div key={repair.id} draggable onDragStart={() => handleDragStart(index)} onDragOver={handleDragOver} onDrop={() => handleDrop(index)} onDragEnd={handleDragEnd}
                  className={`p-4 transition-all duration-200 cursor-move ${draggedIndex === index ? 'opacity-50' : ''}`}
                  style={{ background: '#1a1510', border: '1px solid #2a2018', borderLeft: repair.published ? '2px solid #16a34a' : '2px solid #e80e19' }}
                >
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-bold text-xl mb-1" style={{ color: '#faf8f5', fontFamily: 'var(--font-cormorant)' }}>{repair.name}</h4>
                      {repair.enName && <h5 className="text-md italic" style={{ color: '#c4b49a' }}>{repair.enName}</h5>}
                    </div>
                    <div className="flex flex-col gap-2 items-end">
                      <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider" style={{ background: repair.published ? '#16a34a' : '#e80e19', color: '#faf8f5' }}>
                        {repair.published ? 'PUBLIKOVANY' : 'NEPUBLIKOVANY'}
                      </span>
                      <div className="flex gap-2">
                        <span className="px-2 py-1 text-xs font-bold" style={{ background: '#2a2018', color: '#9b8f84' }}>#{repair.order}</span>
                        <span className="px-2 py-1 text-xs" style={{ background: '#2a2018', color: '#9b8f84' }}>{new Date(repair.createdAt).toLocaleDateString('sk-SK')}</span>
                      </div>
                    </div>
                  </div>

                  <div className="mb-3">
                    <p style={{ color: '#faf8f5' }} className="mb-2">{repair.description}</p>
                    {repair.enDescription && <p className="italic text-sm" style={{ color: '#c4b49a' }}>{repair.enDescription}</p>}
                  </div>

                  {repair.images && repair.images.length > 0 && (
                    <div className="mb-4">
                      <p className="text-sm mb-2" style={{ color: '#9b8f84' }}>Obrazky ({repair.images.length}):</p>
                      <div className="flex gap-2 overflow-x-auto pb-2">
                        {repair.images.map((imageUrl, index) => (
                          <div key={index} className="relative h-24 w-24 flex-shrink-0" style={{ border: '1px solid #2a2018' }}>
                            <Image src={imageUrl} alt={`${repair.name} - ${index + 1}`} width={96} height={96} style={{ objectFit: 'cover' }} />
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 pt-3" style={{ borderTop: '1px solid #2a2018' }}>
                    <button onClick={() => handleEdit(repair)} className="flex-1 py-2 px-4 font-bold text-sm tracking-[0.15em] uppercase transition-colors cursor-pointer" style={{ fontFamily: 'var(--font-poiret-one)', border: '1px solid rgba(139, 105, 20, 0.4)', color: '#8b6914', background: 'transparent' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#8b6914'; e.currentTarget.style.color = '#faf8f5' }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#8b6914' }}>Upravit</button>
                    <button onClick={() => handleDelete(repair.id)} className="flex-1 py-2 px-4 font-bold text-sm tracking-[0.15em] uppercase transition-colors cursor-pointer" style={{ fontFamily: 'var(--font-poiret-one)', border: '1px solid rgba(232, 14, 25, 0.4)', color: '#e80e19', background: 'transparent' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#e80e19'; e.currentTarget.style.color = '#faf8f5' }} onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#e80e19' }}>Vymazat</button>
                  </div>
                </div>
              ))}
          </div>
